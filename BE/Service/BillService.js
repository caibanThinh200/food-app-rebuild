...
class BillService{
    ...
    static async showListBillService(req,res,next){
        try{
            ...
            let billData = await querryBuilder("Bill").where("idUser",userId).orderBy("created_at","desc").select();
    ...
    static async showProductOfBillService(req,res,next){
        try{
            ...
            let products = await querryBuilder("productList").where("idBill",idBill).select();
   
            const product = products.map(async item =>{
                const productInf = await querryBuilder("product").where("idProduct",item.idProduct).first();
                return productInf
            })
